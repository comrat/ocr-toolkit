...
void ocr_img_proc_apply_mask(ocr_img_info *img, ocr_img_info *mask, bool_op_type type)
{
	int i = 0, j = 0;
	int curr_ind = 0;
	int mask_ind = 0;
	int stride = img->stride;
	int m_width = mask->width;
	int m_height = mask->height;
	int m_stride = mask->stride;
	for (i = 0; i < m_height; i++) {
		for (j = 0; j < m_width; j++) {
			curr_ind = i * stride + j;
			mask_ind = i * m_stride + j;
			switch (type) {
			case XOR :
				img->pix[curr_ind] ^= mask->pix[mask_ind];
				break;
			case OR :
				img->pix[curr_ind] |= mask->pix[mask_ind];
				break;
			...
			case NXOR :
				img->pix[curr_ind] ^= mask->pix[mask_ind];
				img->pix[curr_ind] = ~img->pix[mask_ind];
				break;
			case NOR :
				img->pix[curr_ind] |= mask->pix[mask_ind];
				img->pix[curr_ind] = ~img->pix[mask_ind];
			...
			case NAND :
				...
				break;
			case ANDN :
				if (mask->pix[mask_ind] && img->pix[curr_ind])
					img->pix[curr_ind] = CR_WHITE;
...
void ocr_imgproc_morph_exclude_text(ocr_text_area *source, ocr_text_area *exclude)
{
	...
	for (i = 0; i < size; i++) {
		if (source->pix[i] == CR_BLACK && 
		exclude->pix[i] == CR_WHITE)
